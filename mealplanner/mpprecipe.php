...
if (!defined('MPPRECIPE_VERSION_NUM'))
    define('MPPRECIPE_VERSION_NUM', '6.6.15');
...
{ 
    ...
    $h = mpp_gethostname();
    $u = "http://mealplannerpro.com/api/wordpress/register";
    $r = trim( mpprequest( 'get', $u, array( 'host' => $h ) ) );
...
/**
 * Sends a copy of the recipe to the server and receives in response:
...
function mpprecipe_register_recipe( $recipe )
{
    ...
    $response   = mpprequest( "post", "http://mealplannerpro.com/api/wordpress/saverecipe", $data);
...
function mpprecipe_settings() {
    ...
    if (!empty($_POST)) {
        ...
        $rating_label                      = $p['rating-label'];
        $rating_label_hide                 = $p['rating-label-hide'];
    ...
    #    'Traditional 7'    => 'mpprecipe-design12', Temporarily disabled
    ...
    $nutrition_options   = mpprecipe_print_options( 'mpprecipe_nutrition_style', array('Above' =>'above', 'Below' => 'below', 'Minimal' => 'minimal') );
...
function mpprecipe_iframe_content($post_info = null, $get_info = null) {
    ...
    if ($post_info || $get_info) {
        ...
        if ($get_info["post_id"] && !isset($get_info["add-recipe-button"]) && strpos($get_info["post_id"], '-') !== false) {
            ...
            $cuisine  = mpp_keyvalue( "cuisine", $post_info );
            $type  = mpp_keyvalue( "type", $post_info );
...
<body id="mpprecipe-uploader">
    <form enctype='multipart/form-data' method='post' action='' name='recipe_form'>
        ...
        <div id='mpprecipe-form-items'>
            ...
            <p id='mpprecipe_ingredients'  class='cls'><label>Ingredients <span class='required'>*</span> <small>Put each ingredient on a separate line.  There is no need to use bullets for your ingredients. To add sub-headings put them on a new line beginning with "!". Example will be "!for the dressing:"</small></label><textarea id='ingredients_textbox' name='ingredients'>$ingredients</textarea></label></p>
            <p id='mpprecipe-instructions' class='cls'><label>Instructions <small>Press return after each instruction. There is no need to number your instructions.</small></label><textarea id="instructions_textbox" name='instructions'>$instructions</textarea></label></p>
            ...
            <div id='more-options'>
                ...
                <p class='cls'><label>Summary</label> <textarea id='summary_textbox' name='summary'>$summary</textarea></p>
...
-->
                ...
                <p class='cls'><label>Notes</label> <textarea id='notes_textbox' name='notes'>$notes</textarea></label></p>
...
function mpprecipe_enqueue_styles() {
    ...
    // adding google font
    wp_enqueue_style('lato', "//fonts.googleapis.com/css?family=Lato:400,700,300italic,300,100");

    // Add common stylesheet
    wp_enqueue_style('mpprecipe-common', MPPRECIPE_PLUGIN_DIRECTORY .  'mpprecipe-common.css' );
...
function mpprecipe_print_author( $recipe )
{
    return "<span itemprop='author' itemscope itemtype='http://schema.org/Person'>
                <a href='#' itemprop='url' target='_blank' class='mpp-recipe-author'>
                <span itemprop='name'>
                    $recipe->author
                </span>
                </a>
...
{
    return <<<HTML
<a href="#" onclick="$('#creation_options').toggle()">Toggle Link Builder</a>
<div id='creation_options' style='display:none; padding:15px; background: #eee'>
    <h4>Link Builder</h4>
    Build the link and copy and paste it where needed
    <form id=link_creation_form">
    <br/>
        <label> 
            <input id="link_name"     type="text" placeholder="Link test, e.g. Heinz Keitchup"></input>
            Text
        </label> 
        <br/>
        <label> 
            <input id="link_address"  type="text" placeholder="Don't forget the 'http://',  eg. http://google.com"></input>
            URL
        </label> 
        <br/>
        <br/>
        <label> 
            No Follow
            <input id="link_nofollow" type="checkbox"></input>
        </label> 
    </form>
        <br/>
        <br/>
        <label> 
            <input id="link_output" type="text"></input>
            Link Code
        </label> 
    ...
    if( $show_nutrition && $recipe->server_recipe_id && ($recipe->carbs || $recipe->calories || $recipe->fat || $recipe->protein ) )
    {
        ...
        <div class='nutbox'>
            <div class='nut-toprow'>
                ...
                <img src='$plugindir/eshalogo.jpg' />
                ";
        ...
        $output .="
            ...
            <a class='nut-message' target='_blank' href='http://mealplannerpro.com/recipe/nutrition/$recipe->server_recipe_id'>Click Here For Full Nutrition, Exchanges, and My Plate Info</a>
...
function mpprecipe_format_recipe($recipe) {
    ...
    if( $nutrition_style === 'above' )
        $output .= mpprecipe_print_nutrition( $recipe );

    if( !$recipe->recipe_image )
        $recipe->recipe_image = mpprecipe_get_featured_image( $recipe );
	
	if("mpprecipe-design2" == get_option('mpprecipe_stylesheet') or "mpprecipe-design3" == get_option('mpprecipe_stylesheet') or "mpprecipe-design7" == get_option('mpprecipe_stylesheet') or "mpprecipe-design11" == get_option('mpprecipe_stylesheet') or "mpprecipe-design12" == get_option('mpprecipe_stylesheet') or "mpprecipe-design8" == get_option('mpprecipe_stylesheet') or "mpprecipe-design19" == get_option('mpprecipe_stylesheet')  or "mpprecipe-design21" == get_option('mpprecipe_stylesheet') or "mpprecipe-design19" == get_option('mpprecipe_stylesheet') or "mpprecipe-design4" == get_option('mpprecipe_stylesheet') or "mpprecipe-design5" == get_option('mpprecipe_stylesheet')) {
...
*/
			...
			if("mpprecipe-design2" == get_option('mpprecipe_stylesheet') or "mpprecipe-design4" == get_option('mpprecipe_stylesheet') or "mpprecipe-design7" == get_option('mpprecipe_stylesheet') or "mpprecipe-design11" == get_option('mpprecipe_stylesheet') or "mpprecipe-design12" == get_option('mpprecipe_stylesheet') or "mpprecipe-design8" == get_option('mpprecipe_stylesheet') or "mpprecipe-design19" == get_option('mpprecipe_stylesheet') or "mpprecipe-design21" == get_option('mpprecipe_stylesheet')) {
...
/** Ratings disabled
			if ($recipe->rating != 0) {
                ...
                #TODO: reviewCount disabled, determine final status
				$output .= '<span class="rating rating-' . $recipe->rating . '"><span itemprop="ratingValue">' . $recipe->rating . '/5</span>
                               <!--- <span itemprop="reviewCount">1</span></span> -->
			   </p>';
			}
*/
          ...
          if( $subdomain and get_option('mpprecipe_personalizedplugin') )
          {
              $output .= "
                <center><div id='mpp-buttons-2'>
                  <a target='_blank' href='http://$subdomain.mealplannerpro.com/recipe/recipeBox' class='myrecipe-button mpp-button'>My Recipes</a>
                  <a target='_blank' href='http://$subdomain.mealplannerpro.com/grocery-list' class='mylist-button mpp-button'>My Lists</a>
                  <a target='_blank' href='http://$subdomain.mealplannerpro.com/meal-planning-calendar' class='mycal-button mpp-button'>My Calendar</a>
              </div></center>";
          }
          /* END BUTTON CHANGES */
    ...
    if (strlen($printed_copyright_statement) > 0) {
		$output .= '
            <div id="mpp-printed-copyright-statement" itemprop="copyrightHolder" itemscope itemtype="http://schema.org/Organization">
            <span itemprop="name">'
    ...
    if( $nutrition_style === 'minimal' )
        $output .= mpprecipe_print_nutrition( $recipe ) . '</div>';
    elseif( $nutrition_style === 'below' )
        $output .= '</div>' . mpprecipe_print_nutrition( $recipe );
    else
        $output .= '</div> <!-- end mpp -->';
    ...
    return "
        <div id='mpp-buttons'>
          ...
          <div 
                ...
                onclick = 'zlrPrint( \"mpprecipe-container-$recipe_id\", \"$dir\", \"$stylesheet\", \"$noimage\" ); return false'
                $hide
                >
                Print Recipe
                $button_image
    ...
    if( !empty($excerpts) )
    {
        ...
        foreach( $easyrecipes_conv as $id => $recipe )
        {
            $data = array(
                ...
                'serving_size'  => $recipe->servingSize,  'ingredients'   => $recipe->ingredients,
                ...
                'unsatfat'       => $recipe->unsatfat,  
                'transfat'       => $recipe->transfat,  
...
function mpp_convert_easyrecipe_call( $excerpts )
{
    ...
    $response = mpprequest( "post", 'http://mealplannerpro.com/api/clipper/erparsebatch', $data );
...
function mpp_getnutrition()
{
    ...
    $response     = mpprequest( "post", "http://mealplannerpro.com/api/wordpress/saverecipebatch", $data);
...
function mpprecipe_ao_override_cssexclude($exclude) {
    $css = array(
        ...
        'mpprecipe-design12.css', 'mpprecipe-design13.css', 'mpprecipe-design14.css',
        ...
        'mpprecipe-design21.css', 'mpprecipe-design2.css', 'mpprecipe-design3.css',
